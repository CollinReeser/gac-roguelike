    ...
    for (int x = -1; x < 2; x++) {
        for (int y = -1; y < 2; y++) {
            ...
            if (
                ...
                //encourage the creature to get on a cardinal line with player
                if ( // creature is inline with target
                    (node->x_coord == finish->x_coord && new_node->x_coord == finish->x_coord) ||
                    (node->y_coord == finish->y_coord && new_node->y_coord == finish->y_coord)
                ) {
                    new_node->score_start = new_node->score_start + 10;
                }
                else if ( // creature is not inline, see if staying in the current axis is better or worse
                    ...
                    new_node->score_start = new_node->score_start + 9;
                }
                else {
                    new_node->score_start = new_node->score_start + 11;
                }
